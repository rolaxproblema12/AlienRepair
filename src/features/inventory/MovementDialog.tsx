...
import { movementSchema, type MovementInput } from './schemas';
import type { MovementKind, ProductWithCategory } from './types';
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
const KIND_TABS: { key: MovementKind; label: string; icon: typeof ArrowDownToLine }[] = [
...
const DEFAULT_REASON: Record<MovementKind, string> = {
  entrada: 'Compra a proveedor',
  salida: 'Venta directa',
  ajuste: 'Ajuste de inventario',
};

export default function MovementDialog({ open, onOpenChange, product }: Props) {
  const [kind, setKind] = useState<MovementKind>('entrada');
  const record = useRecordMovement(product.id);
  ...
  const projectedStock =
    kind === 'entrada'
      ? product.stock + Math.abs(Number(quantity) || 0)
      : kind === 'salida'
        ? product.stock - Math.abs(Number(quantity) || 0)
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="rounded-md border border-border bg-muted/30 p-3 text-sm">
          ...
          <div className="text-xs text-muted-foreground">
            SKU {product.sku} · Stock actual: <strong>{product.stock}</strong>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Stock proyectado:{' '}
              <strong
                className={cn(projectedStock < 0 && 'text-destructive')}
              >
                {projectedStock}
              </strong>
            </p>
          ...
          <div className="space-y-1">
            ...
            <Textarea
              id="reference"
              rows={2}
              placeholder="Folio, OS-XXXX, factura..."
              {...register('reference')}
            />
          </div>
